<template>
  <div>
    <nav class="navbar navbar-dark bg-tg">
      <span class="navbar-brand h1">The Game ONLINE（WS仮称）</span>
    </nav>
...
export default Vue.extend({
  ...
  methods: {
    createRoomButton() {
      window.location.href = `/play-ws?playerLimit=${ this.createRoom.playerLimit }&name=${ this.createRoom.name }&roomId=${ this.createRoom.roomId }`
    },
    joinRoomButton() {
      window.location.href = `/play-ws?name=${ this.joinRoom.name }&roomId=${ this.joinRoom.roomId }`
    }
  }
})
</script>
